...
router.get("/thanks", function (req, res, next) { //mobile
  res.render("thanksmobile");
});
router.get("/prev", function (req, res, next) { //mobile
  res.render("prev");
});
router.get("/step1", function (req, res, next) { //mobile
  res.render("step1");
});
router.get("/step2", function (req, res, next) { //mobile
  res.render("step2");
});
router.get('/photo_ori', function (req, res, next) {//ดึงรูปออริ หน้า pic ในเอ้กดี ที่เป็นรูปตอนถ่ายเสดแล้วให้ดูว่าจะเอารูปนี้มั้ย
  var mysort = { _id: -1 };
...
router.get("/frame", function (req, res, next) {
  MongoClient.connect(url, { useUnifiedTopology: true }, function (err, db) { //*****แก้หน้าที่ดึงออกมาให้เป็นหมวด */
    if (err) throw err;
...
router.post("/fileuploadpng", function (req, res, next) { //***เพิ่มลิสบ้อกเลือกหมวด ตรงที่อัพเพิ่มหมวดด้วย */
  var form = new formidable.IncomingForm();
  form.parse(req, function (err, fields, files) {
    var oldpath = files.filetoupload.path;
    var type = fields.types;
    //var newpath = "http://localhost:3000/PNG/" + files.filetoupload.name; //อัพลงโฟล์เดอร์ใหม่ตอนที่เอาลงเซิฟเวอร์จริง
    var newpath = "C:/xampp/htdocs/app/public/PNG/" + files.filetoupload.name; //อัพลงโฟล์เดอร์ใหม่จำลองจากเครื่องตัวเอง  *******เปลี่ยนตรงนิ
    var path = "http://localhost:3000/PNG/" + files.filetoupload.name; //ตัวแปรที่จะอัพลงmongo
    ...
    fs.rename(oldpath, newpath, function (err) {
      if (err) throw err;
        ...
        MongoClient.connect(url, function (err, db) {
          ...
          dbo.collection("png").insertOne(myobj, function (err, res) { //ชื่อcollection
            if (err) throw err;
            else {
...
router.post("/fileuploadframe", function (req, res, next) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    //console.log(type);
    //var newpath = "http://localhost:3000/PNG/" + files.filetoupload.name; //อัพลงโฟล์เดอร์ใหม่ตอนที่เอาลงเซิฟเวอร์จริง
    var newpath = "C:/xampp/htdocs/app/public/frame/" + files.filetoupload.name; //อัพลงโฟล์เดอร์ใหม่จำลองจากเครื่องตัวเอง ****เปลี่ยนตรงนี้ๆๆๆแค่นั้น
    var path = "http://localhost:3000/frame/" + files.filetoupload.name; //ตัวแปรที่จะอัพลงmongo
    var name = files.filetoupload.name; //เก้บชื่อ
    fs.rename(oldpath, newpath, function (err) {
      if (err) throw err;
      else {
        MongoClient.connect(url, function (err, db) {
          ...
          var dbo = db.db("pic_mirror"); //ชื่อดาต้าเบส
          var myobj = { name: name, path: path,types:type }; //อัพชื่อกับพาท
          dbo.collection("frame").insertOne(myobj, function (err, res) { //ชื่อcollection
            if (err) throw err;
            else {
              console.log("1 document inserted");
...
router.post("/deco", urlencodedParser, function (req, res, next) { ///เอารูปที่ใส่กรอบแล้วเข้าดาต้าเบส picframe
  ...
  MongoClient.connect(url, { useUnifiedTopology: true }, function (err, db) {
    ...
    var dbo = db.db("pic_mirror"); //ชื่อดาต้าเบส
    //var myobj = { data }; //อัพpic
    dbo.collection("picframe").insertOne(data, function (err, res) { //ชื่อcollection
      if (err) throw err;
      else {
        console.log("1 document inserted");
      }
...
router.get("/dec", function (req, res, next) { //ดึงรูปมาแสดงในหน้าใส่ติ้กเก้อ
  MongoClient.connect(url, { useUnifiedTopology: true }, function (err, db) {
    if (err) throw err;
    var dbo = db.db("pic_mirror");  //แก้ให้ดึงจากลิ้งจริงแล้ว อันเก่าคือ mydb
    dbo
...
router.post("/print", urlencodedParser, function (req, res, next) { ///เอารูปที่ใส่ติ้กแล้วเข้าดาต้าเบส picprint
  const data = req.body; //รับมาจากหน้าframe
  const x = 5;
  console.log(data);
  MongoClient.connect(url, { useUnifiedTopology: true }, function (err, db) {
    ...
    var dbo = db.db("pic_mirror"); //ชื่อดาต้าเบส
    //var myobj = { data }; //อัพpic
    dbo.collection("picprint").insertOne(data, function (err, res) { //ชื่อcollection
      if (err) throw err;
      else {
        console.log("1 document inserted");
...
router.post("/print1", urlencodedParser, function (req, res, next) { ///เอารูปที่ใส่ติ้กแล้วเข้าดาต้าเบส picprint
  const data = req.body; //รับมาจากหน้าframe
  const x = 10;
  console.log(data);
  MongoClient.connect(url, { useUnifiedTopology: true }, function (err, db) {
...
router.post("/frame1", urlencodedParser, function (req, res, next) { ///เอารูปที่ใส่ติ้กแล้วเข้าดาต้าเบส picprint
...
router.get("/preview", function (req, res, next) { //ดึงรูปมาแสดงในหน้าพรีวิว
  MongoClient.connect(url, { useUnifiedTopology: true }, function (err, db) {
    if (err) throw err;
    var dbo = db.db("pic_mirror");  //แก้ให้ดึงจากลิ้งจริงแล้ว อันเก่าคือ mydb
    var mysort = { _id: -1 }; //new to old
